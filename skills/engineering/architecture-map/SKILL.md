...
You will produce **one self-contained HTML file** (default: `./architecture-map.html`) that renders this codebase as an interactive graph. No build step, no framework, no external assets, open the file directly, or serve with `uv run -m http.server 4747`.
...
3. How to view it: open the file directly, or `uv run -m http.server 4747` and visit `http://localhost:4747/architecture-map.html`.
